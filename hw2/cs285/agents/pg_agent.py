...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        """
            ...
            Input: list of rewards {r_0, r_1, ..., r_t', ... r_T} from a single rollout of T T
        ...
        result = np.sum(gammas * rewards)
        list_of_discounted_returns = np.repeat(result, T)
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for t in range(T):
            list_of_discounted_cumsums[t] = np.sum(np.multiply(gammas[:T-t], rewards[t:]))
